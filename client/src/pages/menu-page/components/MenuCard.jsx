const MenuCard = ({ image1, image2, data, title, brunch_className }) => {
  return (
    <div className="relative md:px-20 py-3 mb-[15px] sm:mx-7 xs:mx-0">
      <div className="relative md:mx-10 mx-3 border border-white rounded-lg shadow-lg">
        <div
          className={`${brunch_className} md:w-40 w-20 md:h-40 h-20 absolute right-[-10px] -top-[2rem]`}
        >
          <img className="w-full h-full object-contain" src={image1} alt="" />
        </div>
        <div className="md:w-40 w-20 md:h-40 h-20 absolute md:top-[-4rem] top-[-10px] md:left-[-40px] left-[-20px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:px-10 pb-10">
          {data.map((category) => (
            <div key={category._id}>
              <div className="flex justify-between items-center py-2 px-4 rounded-md">
                <p className="text-lg sm:text-2xl text-white">{category.title}</p>
